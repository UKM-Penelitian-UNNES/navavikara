      ...
      <style>
        ...
        @media (prefers-color-scheme: dark) {
          ...
          ::slotted(*) {
            ...
            color: #f4f4f4;
            margin: 0 auto;
            max-width: calc(var(--slotted-max-width, 100%) + 150px);
          }
...
class UkmpApp extends PolymerElement {
  constructor() {
    ...
    // Gunakan event "DOMContentLoaded" yang lebih cocok untuk elemen yang di-load setelah HTML siap.
    window.addEventListener("DOMContentLoaded", () => {
      const loadElement = this.shadowRoot.querySelector("ukmp-load");

      if (loadElement) {
        loadElement.remove(); // Menghapus dengan cara lebih aman.
      }

      // Menambahkan overflow hidden ke body, jika ingin menambahkannya.
  ...
  static get template() {
    ...
    <ukmp-container>
      ...
      <ukmp-content>
        <div slot="kartu-spotlight">
          <kartu-spotlight></kartu-spotlight>
        </div>
        <div slot="kartu-dep">
          <kartu-dep></kartu-dep>
        </div>
        <!-- Tambahkan konten utama pada elemen "ukmp-content" -->
        <div aplikasi class="apps" slot="kartu-apps">
          <kartu-apps></kartu-apps>
        </div>
        <div slot="kartu-story">
          <kartu-story></kartu-story>
        </div>
        <div prestasi slot="kartu-prestasi">
...
class UkmpContent extends PolymerElement {
  static get template() {
    ...
    <div id="container">
      <slot name="kartu-spotlight"></slot>
...
class UkmpHeaderContent extends HTMLElement {
  ...
  render() {
    ...
    <a href=https://www.ukmpenelitianunnes.com/ id=msLogo class="header_msLogo__PFnQS headerButton" tabindex=0>
        ...
        href=https://ukmpenelitianunnes.com /><span><ukmp-text text="Beranda"></ukmp-text></span></a>
...
class SlideShowSpotlight extends PolymerElement {
  static get properties() {
    return {
      slides: {
        type: Array,
        value: () => [
          {
            ...
            description: 'The all new Microsoft Flight Simulator 2024. Get it here...',
            image: 'https://images-eds-ssl.xboxlive.com/image?url=deg3hPIHDK8R28pOigLGX6ta_nSTtCY41RdKWWhZeeUyV9F7jFUJ8tKfkJT_ZGAOodlutU9udIx6FL1chvIndEHmPKM6jjAJm_NlRVDyvnCnO_aMkQqASQn_jSMzfMah9r3md8mzcIlfvQ_dlTQ_ok2lJcO24gDrnCT3J5niFcguDCS3IquY2T6ZQ5VDLYi5IcRtxgi8lNEljstmPg6YRpPCAhuHlNneZzaFi..6VBM-&h=1080',
            buttonText: 'Get',
            included: 'Included with Game Pass'
          },
          {
            title: 'Join PC Game Pass',
            description: '',
            image: 'https://images-eds-ssl.xboxlive.com/image?url=7flt5HU26ZSS3Tgted_TMty0wzqMQYpm03yD7eAPRtQBYO5dMlD18uZxNDuKXvpqiD2ZNB.J3Iq7tYtR2P.XOdlJuX.sfWhTw3NdvfqIAwKTaUu3gDtqMhb3onBmeCT8otKfNbjgdPvU25ytWuonqY1mt1ojFn8iifqjSpTCxaIHy6I7f8vvABsjoi0UyIDMU8PfkkrvI00w7R.sYqbnaAYR.yYtFM0rCOPmHSRsgVk-&h=1080',
            buttonText: 'Get'
          }
  ...
  static get template() {
    return html`
      ...
      <card-slide-control slides="[[slides]]" current-slide="{{currentSlide}}"></card-slide-control>
      </div>
      <card-button-control on-get-clicked="_handleGetClick"></card-button-control>
    `;
  }

  constructor() {
    super();
    this._boundAutoSlide = this._autoSlide.bind(this);
  }
  ...
  _createSlides() {
    ...
    this.slides.forEach((slide, index) => {
      ...
      slideElement.innerHTML = `
        <image-ui-model class="slide-image" src="${slide.image}" alt="${slide.title}" show-sinematik>
        <div class="slide-content">
          ${slide.included ? `<div class="included-tag">${slide.included}</div>` : ''}
          <h2>${slide.title}</h2>
  ...
  _startSlideshow() {
    this._slideshowInterval = setInterval(this._boundAutoSlide, 5000);
  }
  ...
  _updateActiveSlide() {
    ...
    // Set the new background light based on the active slide
    const activeSlideImage = this.slides[this.currentSlide].image;
  }
  ...
  _handleGetClick() {
    // Implement the action for the "Get" button here
    console.log('Get button clicked for slide:', this.currentSlide);
  }
}

customElements.define('kartu-spotlight', SlideShowSpotlight);


class CardSlideControl extends PolymerElement {
  static get properties() {
    return {
      slides: Array,
      currentSlide: {
  ...
  _createDots() {
    const container = this.shadowRoot.getElementById('dots-container');
    this.slides.forEach((_, index) => {
      const dot = document.createElement('span');
      dot.className = `dot ${index === 0 ? 'active' : ''}`;
      dot.addEventListener('click', () => this._handleDotClick(index));
...
customElements.define('card-slide-control', CardSlideControl);

class CardButtonControl extends PolymerElement {
  static get template() {
    return html`
      <style>
        button {
      ...
      <button on-click="_handleClick">Get</button>
    `;
  }

  _handleClick() {
    this.dispatchEvent(new CustomEvent('get-clicked', { bubbles: true, composed: true }));
  }
}

customElements.define('card-button-control', CardButtonControl);
...
class kartuPres extends PolymerElement {
  ...
  static get template() {
    ...
    <style>
      ...
      <ukmp-header-card-prestasi class="header"></ukmp-header-card-prestasi>
      <div class="card-grid">
        <div class="left-column">
          <div class="card-description">Prestasi UKMP</div>
        </div>
        <div class="right-column">
          <ukmp-kartu-story tag="ukmp-kartu-story" title="Prestasi HRD" color-H="black" color-A="black" color-P="black" description="Data Alumni" url="./story/daukmp/" background-Image="./logo/116223589_1017768482011615_7046452088022274670_n.jpg"></ukmp-kartu-story>
          <ukmp-kartu-story tag="ukmp-kartu-story" title="Prestasi Karya" description="PAB UKMP" url="./story/pabukmp/2022/" background-Image="./logo/312086426_5550639028376326_5765999735847858872_n.jpg"></ukmp-kartu-story>
          <ukmp-kartu-story tag="ukmp-kartu-story" title="Prestasi STD" description="Perayaan Hari Raya Nasional" url="./story/hariraya/" background-Image="./logo/ukmp-haribesar-01.jpg"></ukmp-kartu-story>
        </div>
      </div>  
    `;
  }
...
class UkmpHeaderCardPres extends HTMLElement {
  constructor() {
    super();
    this.attachShadow({ mode: "open" });
    this.shadowRoot.innerHTML = `
...
customElements.define("ukmp-header-card-prestasi", UkmpHeaderCardPres);
